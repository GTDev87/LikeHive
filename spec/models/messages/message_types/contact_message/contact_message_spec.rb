...
describe ContactMessage do  
  before(:each) do
    ...
    create_user_contacts([@user, @greg, @reid, @amol, @zac])
  end
  
  def create_user_contacts(user_list)
    user_list.combination(2).each do |user_pair|
      user_pair[0].network.contacts << user_pair[1]
      user_pair[1].network.contacts << user_pair[0]
    end
  end
  ...
  describe "Contact message properties" do
    it "should have type ConnectionMessage" do      
      ...
      contact_message = Factory.build(:contact_message, message_data: Factory.build(:message_data, to: [user]))
  ...
  describe "To username parsing" do
    it "should add existing users to to list" do
      ...
      message.message_data.from = @user
    ...
    it "should not be valid if user does not have other users as contacts" do
      friendless_user = Factory(:user)
      message = Factory.build(:contact_message)
      message.message_data.from = friendless_user
      message.to_username_list = "A, B, C, D"
  ...
  describe "users receiving messages" do
    ...
    it "should be able to give multiple messages to users" do
      ...
      message2.message_data.from = @user
